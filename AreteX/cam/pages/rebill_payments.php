...
if ($item->initial_payment > 0)
    $initial_term = 'your initial payment of $'.number_format($item->initial_payment,2).'. ';
else  {
    if ($item->initial_period == 1)
        $days = 'day';
    else
        $days = 'days'; 
    $initial_term = "your initial trial period of {$item->initial_period} $days.";
}
    

?>
    
<a style="margin-left: 5px; margin-top: 3px;" href="javascript:void(0);" onclick="load_linked_cam_page('rebill_detail','<?php echo $_REQUEST['linked_id']; ?>');"  
class="ui-button  icon_left_button button_link  ui-state-default ui-corner-all"  >
<span class="ui-icon  ui-icon-circle-arrow-w"></span> 
Back</a> <span style="font-size: 85%">This is your purchase and payment history for: <?php echo $item->product_name;  ?>.
